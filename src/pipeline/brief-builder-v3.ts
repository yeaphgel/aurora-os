...
export function buildImageBriefV3(request: BriefBuilderV3Request): ImageBriefV3 {
  ...
  const image2Prompt = [
    ...
    `Build environmental contact around the product: underwater caustics, soft water haze, rim light, cast shadow, refraction, suspended particles, and foreground/background depth layers.`,
    `Use bold native Image 2 typography: "${headlineText}" must be the main headline at 7% to 11% of canvas height; "${productNameText}" must sit near it as the product name; "${subheadText}" may be one or two short lines.`,
    ...
    `Suppress secondary accessories unless they are tiny contextual details; no side accessory should compete with the hero product.`,
