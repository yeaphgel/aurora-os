...
export function buildImageBriefV2(request: BriefBuilderV2Request): ImageBriefV2 {
  ...
  const headline = input.contentIntent || "Underwater smartphone ecosystem for focused ocean creators";
  const supporting =
    "Preserve the product from the reference image while integrating it into a premium ocean creator scene.";
  const textBlocks: TextBlockSpec[] = [
    ...
    { role: "headline", text: headline, priority: "recommended", editPolicy: "locked_meaning" },
    { role: "supporting", text: supporting, priority: "optional", editPolicy: "creative" },
  ];
  ...
  const userPrompt = [
    ...
    `Visual type: use_case_proof. Story mode: proof_by_context.`,
    `Intent: ${headline}.`,
  ].join(" ");
  ...
  return {
    ...
    logoSpec: {
      ...
      strategy: "native_with_exactness_qa",
